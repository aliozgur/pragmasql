...
namespace MWControls
{
	...
	public class EditorTextDir : System.Drawing.Design.UITypeEditor
	{
		...
		{
      Rectangle result = rct;
      switch(td)
			{
				case MWCommon.TextDir.Normal:
					result = new Rectangle(rct.X + 1, rct.Y, rct.Width, rct.Height);
					break;

				case MWCommon.TextDir.UpsideDown:
          result = new Rectangle(rct.X - 2, rct.Y - 1, rct.Width, rct.Height);
					break;

				case MWCommon.TextDir.Left:
          result = new Rectangle(rct.X - 5, rct.Y + 4, rct.Width, rct.Height);
					break;
