...
namespace ICSharpCode.TextEditor
{
	...
	public class TextArea : Control
	{
		...
		/* HACK for improved scrolling smooothness
				...
				*/
		bool noAutoInvalidate = true;
		public bool NoAutoInvalidate
		{
			get
			{
				return noAutoInvalidate;
			}
			set
			{
				noAutoInvalidate = value;
			}
		}
		...
		protected override void OnMouseHover(EventArgs e)
		{
			...
			{
				// MODIFIED by Ali Özgür: If client coordinate is out of bounds we shall not request tooltip
				// This is original call: RequestToolTip(PointToClient(Control.MousePosition));

				Point p = PointToClient(Control.MousePosition);
				if (p.IsEmpty || p.X < 0 || p.Y < 0)
					return;		
				RequestToolTip(p);
			}
			else
