...
namespace ICSharpCode.TextEditor.Util
{
	...
	{		
		...
		private IncrmentalSearchMessageDelegate _onIncrementalSearchMessage;
		public event IncrmentalSearchMessageDelegate OnIncrementalSearchMessage
		{
			add { _onIncrementalSearchMessage += value; }
			remove { _onIncrementalSearchMessage -= value; }
		}

		private EventHandler _onIncrementalSearchStopped;
		public event EventHandler OnIncrementalSearchStopped
		{
			add { _onIncrementalSearchStopped += value; }
			remove { _onIncrementalSearchStopped -= value; }
		}

		public string SearchText
		{
			get { return searchText.ToString(); }
		}
		...
		public IncrementalSearch(TextEditorControl textEditor, bool forwards)
		{
			...
			if (forwards) {
				incrementalSearchStartMessage = "Inc. Search: ";
			} else {
				incrementalSearchStartMessage = "Reverse Inc. Search: ";
			}
		...
		void StopIncrementalSearch()
		{
			...
			if (_onIncrementalSearchStopped != null)
				_onIncrementalSearchStopped(this, EventArgs.Empty);
		}
		...
		void ShowTextFound(string find)
		{
			if (passedEndOfDocument) {
				ShowMessage("End of document reached.",false);
			} else {
		...
		void ShowMessage(string message, bool highlight)
		{
			...
			if (_onIncrementalSearchMessage != null)
				_onIncrementalSearchMessage(fullMessage, highlight);
		}
		...
		{
			if (_onIncrementalSearchMessage != null)
				_onIncrementalSearchMessage(String.Empty, false);
		}
		...
		Cursor GetCursor()
		{
			...
			{
				if (forwards)
					cursor = new Cursor(new MemoryStream(Properties.Resources.IncrementalSearchCursor));
				else
					cursor = new Cursor(new MemoryStream(Properties.Resources.ReverseIncrementalSearchCursor));				
			}
			return cursor;
		}
	...
	public delegate void IncrmentalSearchMessageDelegate(string msg, bool highlight);
}
