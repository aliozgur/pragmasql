...
namespace PragmaSQL
{
  ...
  /// <summary>
  /// Modified version of ICSharpCode.TextEditor.Document.TextEditorUtilities class.
  /// </summary>
	
  public sealed class SharpDevelopTextEditorUtilities
	{
		/// <remarks>
    ...
    public static bool IsLetterDigitOrSymbol(char c)
    {
    ...
    /*
		...
		*/
    public static bool IsLetterDigitOrValidSymbol(char c)
		{
			if(!Char.IsLetterOrDigit(c)) {
				return ( c == '_' || c == '@' || c == '#' || c == '-');
			}
...
//						break;
					...
					default:
						...
						int start = offset - 1;
						if (!IsLetterDigitOrValidSymbol(document.GetCharAt(start))) {
							goto done;
						}
						
						while (start > 0 && IsLetterDigitOrValidSymbol(document.GetCharAt(start - 1))) {
							--start;
						}
						...
						if (word.Length > 0 && !IsLetterDigitOrValidSymbol(word[0])) {
							goto done;
						}
						offset = start;
						break;
		...
		{
			if(IsLetterDigitOrValidSymbol(c))
		...
		public static int FindWordEnd(IDocument document, int offset)
		{
			...
			while (offset < endPos && IsLetterDigitOrValidSymbol(document.GetCharAt(offset))) {
				++offset;
			}
		...
		public static int FindWordStart(IDocument document, int offset)
		{
			...
			while (offset > line.Offset && !IsLetterDigitOrValidSymbol(document.GetCharAt(offset - 1))) {
				--offset;
			}
		...
		{
			return ( IsLetterDigitOrValidSymbol(ch) );  
      //Commented out by Ali Özgür 
